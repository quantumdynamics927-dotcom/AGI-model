...
def build_connectivity(sequences: Dict[str, str], phi_threshold: float = 0.95, strengthen: float = 0.2) -> Tuple[np.ndarray, Dict[str, Any]]:
    ...
    metadata = {
        ...
        'shape': mat.shape,
        'hash': hashlib.sha256(json.dumps(sequences, sort_keys=True).encode()).hexdigest()
    }
    return mat, metadata


def save_connectivity(mat: np.ndarray, metadata: Dict[str, Any], out_dir: str = 'node12_out', allow_raw: bool = False) -> Path:
    ...
    if allow_flag:
        ...
        logger.info('Raw connectivity emission disabled; set ALLOW_RAW_CONNECTIVITY=1 to enable')
        return meta_path


def stream_connectivity(sequences: Dict[str, str], out_dir: str = 'node12_out', phi_threshold: float = 0.95, strengthen: float = 0.2, allow_raw: bool = False):
    mat, meta = build_connectivity(sequences, phi_threshold=phi_threshold, strengthen=strengthen)
