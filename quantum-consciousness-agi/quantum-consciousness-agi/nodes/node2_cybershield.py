...
def _store_raw_copy(file_path: str, dest_dir: str = "vault/gatekeeper_logs") -> str:
    p = Path(file_path)
    ts = datetime.utcnow().strftime("%Y%m%dT%H%M%SZ")
    dest = Path(dest_dir) / f"raw_{ts}_{p.name}"
    dest.parent.mkdir(parents=True, exist_ok=True)
    shutil.copy2(p, dest)
    return str(dest)


def ingest_scan_file(file_path: str, source: str = "cybershield", ledger: Optional[VaultLedger] = None, sanitize_kwargs: Optional[Dict[str,Any]] = None) -> Dict[str, Any]:
    ...
    entry = ledger.record_operation("CYBERSHIELD_SCAN_RAW", raw_copy, metadata={"source": source, "raw_hash": raw_hash})

    # 2) sanitize via gatekeeper (records GATEWAY_INGESTION in ledger)
    gate = UniversalEntrance(ledger=ledger)
    sanitized = gate.process_external_data(file_path, source_name=source, **sanitize_kwargs)
    ...
    logger.info(f"Ingested scan file. sanitized={result['sanitized_path']} ledger_index={ledger_index}")
    return result


def ingest_scan_payload(payload: Dict[str, Any], source: str = "cybershield", ledger: Optional[VaultLedger] = None, sanitize_kwargs: Optional[Dict[str,Any]] = None) -> Dict[str, Any]:
    """Ingest a payload (dict) by writing to a temp CSV and calling ingest_scan_file."""
    # write single-row CSV
    df = pd.DataFrame([payload])
    tmp_dir = Path("vault/gatekeeper_tmp")
    tmp_dir.mkdir(parents=True, exist_ok=True)
    ts = datetime.utcnow().strftime("%Y%m%dT%H%M%SZ")
    tmp_path = tmp_dir / f"payload_{ts}.csv"
    df.to_csv(tmp_path, index=False)
    return ingest_scan_file(str(tmp_path), source=source, ledger=ledger, sanitize_kwargs=sanitize_kwargs)
