...
class UniversalEntrance:
    def __init__(
        ...
        self.gate_dir = Path(gate_dir) if gate_dir is not None else vault_path("gatekeeper_logs")
    ...
    def process_external_data(
        self,
        file_path: str,
        source_name: str = "External_Input",
        output_dir: str | Path | None = None,
        **sanitize_kwargs,
    ) -> dict:
        file_path = str(file_path)
        logger.info(f"GATEWAY: processing {file_path} (source={source_name})")
        sanitized_output_dir = Path(output_dir) if output_dir is not None else vault_path("sanitized")
        ...
        report = sanitize_dataset(input_path=file_path, output_dir=str(sanitized_output_dir), **sanitize_kwargs)
...
if __name__ == "__main__":
    ...
    p.add_argument("--output", default=str(vault_path("sanitized")))
