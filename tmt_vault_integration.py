...
def _candidate_repo_paths(anchor: Optional[Path] = None) -> Iterable[Path]:
    base = (anchor or Path(__file__).resolve()).parent
    env_value = os.environ.get("TMT_QUANTUM_VAULT_REPO")
    if env_value:
...
def resolve_vault_repo_path(repo_path: Optional[str] = None) -> Path:
    ...
    raise VaultIntegrationError(
        "Could not locate TMT_Quantum_Vault. Set TMT_QUANTUM_VAULT_REPO or clone "
        "the repo beside AGI-model."
    )
