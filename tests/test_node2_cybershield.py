...
MODULE_PATH = ROOT / "TMT-OS" / "node2_cybershield.py"

if not MODULE_PATH.exists():
    pytest.skip(
        "TMT-OS/node2_cybershield.py not present in this repo", allow_module_level=True
    )

spec = importlib.util.spec_from_file_location("node2_cybershield", MODULE_PATH)
node2_module = importlib.util.module_from_spec(spec)
assert spec.loader is not None
spec.loader.exec_module(node2_module)
...
class TestNode2CyberShield(unittest.TestCase):
    ...
    def test_hmac_signature(self):
        ...
        self.assertFalse(
            self.shield.verify_hmac_signature(data, "incorrect_signature_string" * 3)
        )

        # Test 3: Tampered data with a valid signature should fail.
        self.assertFalse(
            self.shield.verify_hmac_signature(b"tampered payload data", signature)
        )
    ...
    def test_access_control_logic(self):
        ...
        self.assertFalse(
            self.shield.check_access("node", "delete_everything")
        )  # An action not in its list
    ...
    def test_audit_log_creation_and_content(self):
        ...
        self.assertTrue(
            os.path.exists(self.audit_log_path), "Audit log file should be created."
        )
    ...
    def tearDownClass(cls):
        ...
        if Path(cls.audit_log_path).exists():
            try:
                Path(cls.audit_log_path).unlink()
            except OSError as e:
