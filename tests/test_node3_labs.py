...
Node3ExperimentalLabs = pytest.importorskip(
    "tmt_os_labs.node3_experimental_labs"
).Node3ExperimentalLabs
...
class TestNode3ExperimentalLabs(unittest.TestCase):
    ...
    def test_record_measurement_invalid_experiment(self):
        ...
        success = self.node.record_measurement(
            "invalid_exp", "control", "accuracy", 0.85
        )
    ...
    def test_record_measurement_invalid_variant(self):
        ...
        exp_id = self.node.create_experiment(
            name="Test", description="Test", variants=["A", "B"], metrics=["accuracy"]
        )

        success = self.node.record_measurement(
            exp_id, "invalid_variant", "accuracy", 0.85
        )
        self.assertFalse(success)
