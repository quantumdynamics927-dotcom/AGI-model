"""Tests for the improved wing occlusion module."""
...
wing_occlusion_path = (
    Path(__file__).parent.parent / "TMT-OS" / "wing_occlusion" / "advanced_occlusion.py"
)
if not wing_occlusion_path.exists():
    pytest.skip(
        "advanced_occlusion.py not present in this repo",
...
def test_golden_ratio_validation(demonstrator):
    ...
    assert demonstrator.validate_golden_ratio()

    # Should fail with wrong phi
    demonstrator.phi = 2.0
    assert not demonstrator.validate_golden_ratio()

    # Should pass with tolerance
    demonstrator.phi = PHI + 0.0001
    assert demonstrator.validate_golden_ratio(tolerance=0.001)
...
def test_multiple_runs_consistency(demonstrator):
    ...
    np.testing.assert_array_equal(results1["original_data"], results2["original_data"])
